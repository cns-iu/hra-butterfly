...
# Fetch ASCT+B data with defensive checks so we can surface non-JSON responses
resp = requests.get(ASCTB_DATA, timeout=30)
if resp.status_code != 200:
    print(f"ERROR: HTTP {resp.status_code} fetching {ASCTB_DATA}")
...
try:
    ...
    print("ERROR decoding JSON from", ASCTB_DATA, ":", e)
